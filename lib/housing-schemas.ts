...
export const housingListingUpdateSchema=z.object({
  action:z.enum(["update","hide","archive","reopen","renew","occupied","found"]), version:z.number().int().positive(), listingType:z.enum(housingListingTypes).optional(), category:z.enum(housingCategories).optional(), title:z.string().trim().min(5).max(120).optional(), locality:z.string().trim().min(2).max(100).optional(), availableFrom:z.iso.date().optional(), stayLength:z.enum(housingStayLengths).optional(), shortDescription:z.string().trim().min(20).max(240).optional(), description:z.string().trim().min(50).max(4000).optional(), priceMonthly:z.number().int().min(0).max(200000).optional(), utilitiesIncluded:z.boolean().optional(), utilitiesAmount:z.number().int().min(0).max(100000).nullable().optional(), depositAmount:z.number().int().min(0).max(500000).nullable().optional(), availableSpots:z.number().int().min(1).max(20).nullable().optional(), currentOccupants:z.number().int().min(0).max(30).nullable().optional(), furnished:z.boolean().nullable().optional(), transitAccess:z.string().trim().max(160).optional(), features:z.array(z.enum(housingFeatures)).max(8).optional(), wantedPersonCount:z.number().int().min(1).max(10).nullable().optional(), lifestylePreferences:z.array(z.enum(housingLifestylePreferences)).max(6).optional(),
