...
export type TableName = "cities" | "campuses" | "service_requests" | "submissions" | "outbound_clicks" | "page_views" | "academic_events" | "community_events" | "places" | "offers" | "jobs" | "content_sources" | "source_sync_runs" | "source_review_queue" | "link_checks" | "content_publication_events" | "buddy_posts" | "buddy_join_requests" | "content_reports" | "contact_messages" | "academic_event_conflicts" | "community_profiles" | "community_posts" | "community_comments" | "community_reactions" | "community_reports" | "community_moderation_history" | "community_moderation_settings" | "anonymous_installations" | "saved_items" | "internal_notifications" | "push_subscriptions" | "calendar_subscriptions" | "community_event_interests" | "place_live_reports" | "academic_event_changes" | "notification_deliveries" | "moderation_actions" | "marketplace_listings" | "marketplace_listing_photos" | "marketplace_messages" | "marketplace_reports" | "marketplace_history" | "marketplace_moderation_actions" | "marketplace_abuse_blocks" | "marketplace_rate_limits" | "chat_conversations" | "chat_conversation_members" | "chat_messages" | "chat_message_reports" | "chat_moderation_actions" | "chat_rate_limits";
...
async function mutateLocalStore<T>(mutation: (store: LocalStore) => T | Promise<T>) {
  const operation = localMutationQueue.then(async () => { const store = await readLocalStore(); const result = await mutation(store); await writeLocalStore(store); return result; });
...
export async function insertRecord(table: TableName, record: RecordValue) {
  ...
  assertStorage(); return mutateLocalStore((store) => { const saved = { id: record.id || crypto.randomUUID(), created_at: new Date().toISOString(), ...record }; store[table].push(saved); return saved; });
}
export async function listRecords(table: TableName) {
  ...
  if (!localStoreAllowed()) return []; await localMutationQueue; return (await readLocalStore())[table];
}
export async function updateRecord(table: TableName, id: string, changes: RecordValue) {
  ...
  assertStorage(); return mutateLocalStore((store) => { const index = store[table].findIndex((record) => String(record.id) === id); if (index < 0) throw new Error("Záznam nebyl nalezen."); store[table][index] = { ...store[table][index], ...changes, updated_at: new Date().toISOString() }; return store[table][index]; });
}
export async function deleteRecord(table: TableName, id: string) {
  ...
  assertStorage(); await mutateLocalStore((store) => { store[table] = store[table].filter((record) => String(record.id) !== id); });
}
