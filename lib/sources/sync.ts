...
export async function eventFingerprint(event: NormalizedEvent) { return sha256([event.universityId, event.facultyId, event.academicYear, semesterFor(event), event.category, foldSearchText(event.title)].join("|")); }
...
export async function syncSource(sourceId: string, cityId?: string, options: { claimed?: boolean } = {}) {
  ...
  try {
    ...
    const { data: storedSource } = await client.from("content_sources").select("etag,last_modified,content_hash,normalized_hash,last_document_url,requires_review,sync_status,enabled,next_deep_discovery_at").eq("id", source.id).single();
    const deepDiscovery = !storedSource?.next_deep_discovery_at || new Date(String(storedSource.next_deep_discovery_at)).getTime() <= Date.now();
    const { fetched, effectiveSource, discovered, discoveryPerformed } = await fetchSourcePayload(
      source,
      { etag: storedSource?.etag, lastModified: storedSource?.last_modified },
      new Date(),
      { knownDocumentUrl: storedSource?.last_document_url, deepDiscovery },
    );
    const discoveryPatch = discoveryPerformed ? {
      last_deep_discovery_at: new Date().toISOString(),
      next_deep_discovery_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(),
      discovery_status: discovered ? "public_source_found" : "public_source_not_found",
      discovery_candidate_count: discovered ? 1 : 0,
      discovery_summary: { document_url: discovered?.url || null, document_title: discovered?.title || null, academic_year: discovered?.academicYear || null, kind: discovered?.kind || "unknown" },
    } : {};
    if (fetched.status === 304) { const finishedAt = new Date().toISOString(); if (!storedSource?.requires_review) await markPublishedEventsVerified(client, source.id, finishedAt); await client.from("content_sources").update({ last_checked_at: finishedAt, last_success_at: finishedAt, last_http_status: 304, consecutive_failures: 0, sync_status: storedSource?.requires_review ? "manual_review" : "not_modified", next_check_at: nextCheckAt(finishedAt), next_retry_at: null, last_error_message: null, ...discoveryPatch }).eq("id", source.id); await client.from("source_sync_runs").update({ status: "not_modified", finished_at: finishedAt, http_status: 304 }).eq("id", run.id); return { sourceId, status: "not_modified" as const }; }
    const contentHash = await sha256(fetched.body); if (contentHash === storedSource?.content_hash) { const finishedAt = new Date().toISOString(); if (!storedSource?.requires_review) await markPublishedEventsVerified(client, source.id, finishedAt); await client.from("content_sources").update({ last_checked_at: finishedAt, last_success_at: finishedAt, last_http_status: fetched.status, etag: fetched.etag, last_modified: fetched.lastModified, consecutive_failures: 0, sync_status: storedSource?.requires_review ? "manual_review" : "not_modified", next_check_at: nextCheckAt(finishedAt), next_retry_at: null, last_error_message: null, ...discoveryPatch }).eq("id", source.id); await client.from("source_sync_runs").update({ status: "not_modified", finished_at: finishedAt, http_status: fetched.status, content_hash: contentHash }).eq("id", run.id); return { sourceId, status: "not_modified" as const }; }
