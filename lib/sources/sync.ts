...
export const normalizedEventToRow = (event: NormalizedEvent, approved = event.status === "approved") => ({ external_id: event.externalId, title: event.title, description: event.description, starts_at: event.startAt, ends_at: event.endAt || null, all_day: event.allDay, timezone: event.timezone, category: categoryCode(event.category), academic_year: event.academicYear, university_id: event.universityId, faculty_id: event.facultyId, programme_id: event.programmeId || null, campus_id: event.campusId || null, scope_type: event.programmeId ? "programme" : event.campusId ? "campus" : "faculty", school: event.universityId.toUpperCase(), faculty: event.facultyId, source_id: event.sourceId, source_name: "Oficiální veřejný zdroj", source_url: event.sourceUrl, source_document_title: event.sourceDocumentTitle || null, source_updated_at: event.sourceUpdatedAt || null, source_hash: event.sourceHash, confidence: event.confidence, status: approved ? "approved" : "pending", verification_status: approved ? "verified" : "needs_review", last_verified_at: event.lastVerifiedAt, is_demo: false, is_cancelled: false, change_state: "unchanged" });
...
export async function syncSource(sourceId: string, cityId?: string) {
  ...
  const client = createServiceClient(); const { data: claimed, error: claimError } = await client.rpc("claim_content_source", { source_key: source.id }); if (claimError) throw claimError; if (!claimed) return { sourceId, status: "busy" as const };
  ...
  try {
    ...
    if (fetched.status === 304) { await client.from("content_sources").update({ last_checked_at: new Date().toISOString(), last_success_at: new Date().toISOString(), last_http_status: 304, consecutive_failures: 0, sync_status: "not_modified" }).eq("id", source.id); await client.from("source_sync_runs").update({ status: "not_modified", finished_at: new Date().toISOString(), http_status: 304 }).eq("id", run.id); return { sourceId, status: "not_modified" as const }; }
    const contentHash = await sha256(fetched.body); if (contentHash === storedSource?.content_hash) { await client.from("content_sources").update({ last_checked_at: new Date().toISOString(), last_success_at: new Date().toISOString(), last_http_status: fetched.status, etag: fetched.etag, last_modified: fetched.lastModified, consecutive_failures: 0, sync_status: "not_modified" }).eq("id", source.id); await client.from("source_sync_runs").update({ status: "not_modified", finished_at: new Date().toISOString(), http_status: fetched.status, content_hash: contentHash }).eq("id", run.id); return { sourceId, status: "not_modified" as const }; }
    ...
    const { data: existingRows, error: existingError } = await client.from("academic_events").select("id,external_id,source_hash,manual_override,starts_at,title,is_cancelled").eq("source_id", source.id); if (existingError) throw existingError;
    const partition = partitionEventsForMonitoring(source.monitoringMode, result.events);
    const certain = connectorIssue ? [] : partition.publishable;
    const uncertain = connectorIssue ? result.events : partition.review;
    const reviewWarnings = connectorIssue ? [...result.warnings, connectorIssue.message] : result.warnings;
    const changes = reconcileEvents((existingRows || []) as ExistingEvent[], certain);
    if (certain.length) { const changedIds = new Set(changes.updates.map((event) => event.externalId)); const { error } = await client.from("academic_events").upsert(certain.map((event) => ({ ...normalizedEventToRow(event), change_state: changedIds.has(event.externalId) ? "changed" : "unchanged" })), { onConflict: "source_id,external_id" }); if (error) throw error; }
    if (uncertain.length || reviewWarnings.length) await client.from("source_review_queue").insert({ source_id: source.id, sync_run_id: run.id, proposed_payload: { events: uncertain, warnings: reviewWarnings }, source_text: result.sourceText || uncertain.map((event) => event.originalText).filter(Boolean).join("\n") || null, confidence: uncertain.length ? Math.max(...uncertain.map((event) => event.confidence)) : source.confidence, source_document_title: result.documentTitle || source.sourceDocumentTitle || null, reason: connectorIssue?.code || (uncertain.length ? "low_confidence" : "parser_warning"), status: "pending" });
    const archived = sourceRunMayArchive(source.monitoringMode, { publishableCount: certain.length, reviewCount: uncertain.length, warningCount: reviewWarnings.length, blocked: Boolean(connectorIssue) }) ? changes.archived : [];
    ...
    if (error instanceof SourceBlockedError) {
      ...
      await client.from("content_sources").update({ last_checked_at: finishedAt, requires_review: true, sync_status: "manual_review", consecutive_failures: 0, last_block_reason: message, last_final_url: error.metadata.finalUrl || source.sourceUrl, last_content_type: error.metadata.contentType || null }).eq("id", source.id);
    ...
    await client.from("content_sources").update({ last_checked_at: finishedAt, consecutive_failures: failures, sync_status: failures >= 3 ? "stale" : "failed" }).eq("id", source.id); throw error;
  }
}
