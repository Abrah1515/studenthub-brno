...
export async function parseHtml(context: ConnectorContext): Promise<ConnectorResult> {
  ...
  for (const text of candidates(html)) {
    ...
    events.push({ externalId, title, description: "Událost načtená z veřejného oficiálního harmonogramu.", startAt: parsed.start, endAt: parsed.end, allDay: parsed.allDay, timezone: "Europe/Prague", category, academicYear: academicYearFor(new Date(parsed.start)), universityId: context.source.universityId, facultyId: context.source.facultyId, sourceId: context.source.id, sourceUrl: context.source.sourceUrl, sourceHash: await sha256(text), confidence: 0.92, status: "approved", lastVerifiedAt: context.checkedAt });
  }
  if (!events.length) warnings.push("Parser nenašel žádnou dostatečně jistou událost; dokument čeká na kontrolu.");
  return { events, warnings, sourceText: decodeHtml(html), extractionMethod: "structured" };
}
