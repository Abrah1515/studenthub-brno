...
export function publicMarketplaceListing(row: Record<string, unknown>, photos: Array<Record<string, unknown> & { signedUrl?: string }> = [], includeInactive = false): MarketplaceListing | null {
  ...
  if (!publicStatuses.has(status) && !(includeInactive && ["archived", "expired", "hidden", "rejected", "pending_verification", "pending_review"].includes(status))) return null;
  return {
    ...
    handoffLocation: optionalText(row.handoff_location), publicAlias: String(row.public_alias), author: legacyProfileIdentity, owned: false, chatAvailable: Boolean(row.seller_id && ["active", "reserved"].includes(status)), status: status as MarketplaceListing["status"], publishedAt: String(row.published_at),
