...
- 27 fakult a městské routy `/brno`, `/brno/kalendar`, `/brno/mista`, `/brno/brigady`, `/brno/burza` a `/brno/skoly/<škola>`; původní URL bezpečně přesměrovávají a vypnutá `/brno/nabidky` vede na přehled;
...
| `MARKETPLACE_FROM_EMAIL` | pouze server | ano pro Burzu | ověřená odesílací identita pro ověřovací odkazy a relay; pokud není nastavena, použije se `CONTACT_FROM_EMAIL` |
...
`pnpm test` spouští kromě unit testů také izolovanou PostgreSQL integraci přes PGlite: kontroluje všech 32 migrací, aplikuje datové migrace a seed, zpracuje HTML/PDF fixtures, vytvoří veřejné události i review frontu a ověří RLS rolí `anon`, běžný uživatel, `faculty_editor`, `city_editor` a `super_admin`, publikační oprávnění a jeho audit, veřejný grant profilu bez citlivých sloupců, zákaz přímého analytického zápisu, kapacitu parťáků, soukromý kontaktní inbox, komunitní tok, komunitní místa, úplný databázový tok Burzy a chatový request → přijetí odpovědí → blokace včetně zákazu přístupu třetího profilu. Čistá instalace obsahuje jen ověřené veřejné zdroje a žádné falešné inzeráty ani zprávy. Infrastrukturní migrace `202608110012` a `202608110014` pro `pg_cron`/`pg_net` mají samostatné regresní testy a ověřují se nad propojeným Supabase, protože PGlite tato hostovaná rozšíření neposkytuje.
...
`/brno/mista` rozlišuje 12 kategorií, oficiálně zdrojované a komunitní záznamy a zachovává seznam plně použitelný bez mapy. Návrh může odeslat pouze přihlášený profil s potvrzeným e-mailem, aktivním a dokončeným účtem a přijatými pravidly. Textový koncept se ukládá jen do zařízení a přežije přihlášení; fotografie se z bezpečnostních důvodů musí po návratu vybrat znovu. Server kontroluje HTTPS zdroj, přesný potvrzený bod, honeypot, denní limit a duplicity podle normalizovaného názvu, aliasu, adresy, blízkosti a webu. Obrázky se dekódují bez důvěry v příponu, zmenší a znovu uloží jako WebP do soukromého bucketu `place-submission-images`.

Návrh zůstává v `place_submissions` neveřejný do schválení. Administrátor nebo městský editor může návrh upravit a schválit, vrátit, zamítnout, sloučit či archivovat; fakultní editor tuto městskou frontu nevidí. Schválený nový záznam získá `origin='community'`, viditelný štítek a datum moderace. Identita autora, fotografie před schválením, poznámky a historie nejsou součástí veřejného API. Zkušenosti se zveřejní po serverové validaci ihned, mají nejvýše 600 znaků, nesmějí obsahovat kontakt ani odkaz, lze je upravit, odstranit, označit jako užitečné a nahlásit. Souhrnná vlastnost místa se zobrazí až po hlasech nejméně tří nezávislých profilů a nikdy se nemíchá s oficiální otevírací dobou ani živým stavem.

Vyhledávání adres je ručně spouštěný serverový proxy. Výchozí `GEOCODING_ENDPOINT` používá veřejný Nominatim pouze bez autocomplete, nejvýše jednou za sekundu na instanci, s aplikačním User-Agentem, dlouhou cache, uživatelským limitem a viditelným uvedením OpenStreetMap. Pro vyšší provoz nastavte kompatibilní vlastní nebo smluvní geocoder; endpoint lze změnit bez nové verze klienta. Aktuální podmínky jsou na `https://operations.osmfoundation.org/policies/nominatim/`.
