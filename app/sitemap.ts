...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const global = ["/komunita", "/partak", "/o-projektu", "/kontakt", "/soukromi", "/cookies", "/podminky"].map((path) => ({ url: `${base}${path}`, lastModified: new Date(), changeFrequency: path === "/komunita" ? "daily" as const : "monthly" as const, priority: path === "/komunita" ? .8 : .5 }));
