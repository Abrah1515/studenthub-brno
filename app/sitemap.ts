...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const global = ["/pomoc", "/partak", "/navrhnout-obsah", "/o-projektu", "/kontakt", "/soukromi", "/cookies", "/podminky"].map((path) => ({ url: `${base}${path}`, lastModified: new Date(), changeFrequency: "monthly" as const, priority: .5 }));
