...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const city = await getPublishedCity((await params).city); if (!city) notFound(); return { title: `Kalendář a Co se děje · ${city.name}`, description: `Aktuální školní termíny a veřejné komunitní akce pro studenty ve městě ${city.name}.`, alternates: { canonical: `/${city.slug}/kalendar` } }; }
