...
const schema = z.object({ email: z.string().trim().email(), next: z.string().startsWith("/").max(200).refine((value) => !value.startsWith("//"), "Neplatný návratový odkaz.").optional() });
export async function POST(request: Request) {
  ...
  const next = parsed.data.next || "/partak";
  const { error } = await client.auth.signInWithOtp({ email: parsed.data.email, options: { emailRedirectTo: `${siteUrl}/auth/callback?next=${encodeURIComponent(next)}` } });
