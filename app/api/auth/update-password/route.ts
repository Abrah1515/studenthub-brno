...
export async function POST(request:Request){
  ...
  const response=NextResponse.json({message:"Heslo bylo bezpečně změněno.",next:"/brno/nastaveni"},{headers:{"Cache-Control":"private, no-store"}}); const client=await authRouteClient(response); if(!client) return NextResponse.json({message:"Obnova hesla není dostupná."},{status:503});
  ...
  const body=NextResponse.json({message:"Heslo bylo bezpečně změněno. Ostatní relace byly odhlášeny.",next:administrative?"/admin":"/brno/nastaveni"},{headers:{"Cache-Control":"private, no-store"}});
