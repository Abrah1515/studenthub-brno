...
export async function GET(){ const profile=await getCurrentAccount(); if(!profile) return NextResponse.json({message:"Přihlaste se."},{status:401}); const client=createServiceClient(); const [{data:posts},{data:comments},{data:buddy},{data:listings},{data:events}]=await Promise.all([
...
]); return NextResponse.json({posts:posts||[],comments:comments||[],buddy:buddy||[],listings:listings||[],events:events||[]},{headers:{"Cache-Control":"private, no-store"}}); }
