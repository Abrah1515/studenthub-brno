...
export async function PATCH(request: Request, context: Context) {
  if (!isSupabaseConfigured()) return NextResponse.json({ message: "Úložiště není připojené." }, { status: 503 }); const user = await getCurrentAccount(); if (!user) return NextResponse.json({ message: "Nepřihlášeno." }, { status: 401 }); if (!user.complete || user.accountStatus !== "active") return NextResponse.json({ message: "Profil není připravený pro komunitní akce." }, { status: 403 });
