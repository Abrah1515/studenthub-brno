...
export async function POST(request: Request) {
  if (!isSupabaseConfigured()) return NextResponse.json({ message: "Hlášení vyžaduje připojený Supabase Auth." }, { status: 503 }); const user = await getCurrentUser(); if (!user) return NextResponse.json({ message: "Nejprve ověřte e-mail přihlašovacím odkazem." }, { status: 401 });
