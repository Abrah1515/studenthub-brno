...
export async function GET() {
  ...
  const sourceIds = (sources || []).filter((row) => row.city_id === "brno" || universities.includes(String(row.university_id))).map((row) => String(row.id));
  ...
  return NextResponse.json({ configuration: calendarAiConfiguration(), latest, lastSuccessful: (runs || []).find((row) => row.status === "completed") || null, runs: runs || [], findings: findings || [], stats: { sources: sourceIds.length, openFindings: (findings || []).filter((row) => ["new", "needs_review", "cannot_verify"].includes(String(row.status))).length, unavailable: Number(latest?.unavailable_source_count || 0), conflicts: Number(latest?.conflict_count || 0) } });
}
...
export async function POST(request: Request) {
  ...
  try { return NextResponse.json(await runAcademicCalendarAiCheck({ trigger: "manual", cityId: "brno", actorId: user.id, targetSourceId: parsed.data.sourceId || null })); }
