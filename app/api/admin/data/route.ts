...
export async function GET() {
  ...
  let profiles: Record<string, unknown>[] = []; let profileReports: Record<string, unknown>[] = []; let accountModerationHistory: Record<string, unknown>[] = [];
  if (isSupabaseConfigured() && user.role === "super_admin") {
    const client = createServiceClient(); const [profileResult, reportResult, historyResult] = await Promise.all([
      client.from("profiles").select("id,username,display_name,role,city_id,university_id,faculty_id,study_year,profile_visibility,account_status,suspended_at,created_at,updated_at").order("created_at", { ascending: false }).limit(500),
    ...
    ]); profiles = profileResult.data || []; profileReports = reportResult.data || []; accountModerationHistory = historyResult.data || [];
  }
