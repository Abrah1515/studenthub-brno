...
export async function GET() {
  ...
  const scopedSources = scoped(sources);
  const scopedSourceIds = new Set(scopedSources.map((source) => String(source.id)));
  const scopedSourceRows = (items: Record<string, unknown>[]) => user.role === "super_admin" ? items : items.filter((row) => scopedSourceIds.has(String(row.source_id || row.id)));
  ...
  let housingListings: Record<string, unknown>[] = []; let housingReports: Record<string, unknown>[] = []; let housingHistory: Record<string, unknown>[] = []; let housingActions: Record<string, unknown>[] = []; let housingRuns: Record<string, unknown>[] = [];
  if (isSupabaseConfigured() && user.role !== "faculty_editor") {
    ...
    const [listingResult, housingReportResult, housingHistoryResult, actionResult, runResult] = await Promise.all([
      ...
      housingClient.from("housing_maintenance_runs").select("id,status,expired_count,orphan_photo_count,error_code,started_at,finished_at").order("started_at", { ascending: false }).limit(50),
    ]);
    housingListings = scoped(listingResult.data || []); const housingIds = new Set(housingListings.map((row) => String(row.id)));
    housingReports = (housingReportResult.data || []).filter((row) => housingIds.has(String(row.listing_id))); housingHistory = (housingHistoryResult.data || []).filter((row) => housingIds.has(String(row.listing_id))); housingActions = (actionResult.data || []).filter((row) => !row.listing_id || housingIds.has(String(row.listing_id))); housingRuns = user.role === "super_admin" ? runResult.data || [] : [];
  }
  ...
  const can = (section: Parameters<typeof adminSectionAllowed>[0]) => adminSectionAllowed(section, user.role);
  const visibleOperations = can("analytics") ? operations : { ...operations, installations: 0, activePushSubscriptions: 0, activeCalendarSubscriptions: 0, unreadNotifications: 0, pendingPush: 0, sentPush: 0, failedPush: 0 };
  return NextResponse.json({ _meta: { dataSource: isSupabaseConfigured() ? "supabase" : verifiedFallback ? "verified_fallback" : "unconfigured", verifiedFallback, externalProviders, operations: visibleOperations }, profiles: profilesWithCounts, profile_reports: profileReports, account_moderation_history: accountModerationHistory, profile_permissions: profilePermissions, profile_permission_audit: profilePermissionAudit, cities: can("cities") ? scoped(values.cities) : [], academic_events: can("academic_events") ? scoped(values.academic_events) : [], community_events: can("community_events") ? scoped(values.community_events) : [], places: can("places") ? scoped(values.places) : [], place_live_reports: can("live_reports") ? placeLiveReports : [], moderation_actions: can("live_reports") ? liveModerationActions : [], offers: can("offers") ? scoped(values.offers) : [], jobs: can("jobs") ? scoped(values.jobs) : [], marketplace_listings: can("marketplace") ? marketplaceListings : [], marketplace_reports: can("marketplace") ? marketplaceReports : [], marketplace_history: can("marketplace") ? marketplaceHistory : [], marketplace_moderation_actions: can("marketplace") ? marketplaceActions : [], marketplace_abuse_blocks: marketplaceBlocks, housing_listings: can("housing") ? housingListings : [], housing_reports: can("housing") ? housingReports : [], housing_history: can("housing") ? housingHistory : [], housing_moderation_actions: can("housing") ? housingActions : [], housing_maintenance_runs: can("housing") ? housingRuns : [], service_requests: can("service_requests") ? scoped(values.service_requests) : [], submissions: can("submissions") ? scoped(values.submissions) : [], outbound_clicks: can("analytics") ? scoped(values.outbound_clicks) : [], page_views: can("analytics") ? scoped(values.page_views) : [], content_sources: can("content_sources") ? scopedSources : [], source_sync_runs: can("content_sources") ? scopedSourceRows(values.source_sync_runs) : [], source_review_queue: can("source_review_queue") ? scopedSourceRows(values.source_review_queue) : [], link_checks: can("content_sources") ? scopedSourceRows(values.link_checks) : [], content_publication_events: user.role === "super_admin" || user.role === "admin" ? scoped(values.content_publication_events) : [], buddy_posts: can("buddy_posts") ? scoped(values.buddy_posts) : [], buddy_join_requests: can("buddy_posts") ? privileged(values.buddy_join_requests) : [], content_reports: can("content_reports") ? scoped(values.content_reports) : [], contact_messages: can("contact_messages") ? scoped(values.contact_messages) : [], academic_event_conflicts: can("academic_event_conflicts") ? scopedSourceRows(values.academic_event_conflicts) : [], community_profiles: can("community_forum") ? scoped(values.community_profiles) : [], community_posts: can("community_forum") ? scoped(values.community_posts) : [], community_comments: can("community_forum") ? communityComments : [], community_reports: can("community_forum") ? scoped(values.community_reports) : [], community_moderation_history: can("community_forum") ? scoped(values.community_moderation_history) : [], community_moderation_settings: can("community_forum") ? scoped(values.community_moderation_settings) : [] });
}
