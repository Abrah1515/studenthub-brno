...
export async function POST(request: Request) {
  ...
  const saved = await insertRecord("buddy_posts", { owner_id: profile.id, city_id: city.id, activity_type: parsed.data.activityType, title: parsed.data.title, approximate_location: parsed.data.approximateLocation, starts_at: startsAt.toISOString(), description: parsed.data.description, max_participants: parsed.data.maxParticipants, status: "active", moderation_status: "approved", expires_at: expiresAt.toISOString() });
