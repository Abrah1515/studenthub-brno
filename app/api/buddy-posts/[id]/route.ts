...
export async function PATCH(request: Request, context: Context) {
  ...
  const user = await getCurrentAccount(); if (!user) return NextResponse.json({ message: "Přihlaste se." }, { status: 401 }); if (!user.complete || user.accountStatus !== "active") return NextResponse.json({ message: "Aktivní dokončený profil je povinný." }, { status: 403 });
  ...
  const saved = await updateRecord("buddy_posts", id, { ...(value.activityType ? { activity_type: value.activityType } : {}), ...(value.title ? { title: value.title } : {}), ...(value.approximateLocation ? { approximate_location: value.approximateLocation } : {}), ...(value.description ? { description: value.description } : {}), ...(value.maxParticipants ? { max_participants: value.maxParticipants } : {}), ...(value.status ? { status: value.status } : {}), ...(startsAt ? { starts_at: startsAt.toISOString(), expires_at: new Date(startsAt.getTime() + 12 * 60 * 60 * 1000).toISOString() } : {}) });
...
export async function DELETE(request: Request, context: Context) {
  ...
  await updateRecord("buddy_posts", id, { status: "deleted" }); return new NextResponse(null, { status: 204 });
}
