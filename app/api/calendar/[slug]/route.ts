...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const requestedUniversity = query.get("university"); const requestedFaculty = query.get("faculty"); const category = query.get("category"); const textQuery = query.get("q")?.trim().toLocaleLowerCase("cs-CZ") || "";
  ...
  const events = await getAcademicEvents(city.id, { universityId: resolved.universityId || undefined, facultyId: resolved.facultyId || undefined });
