...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const selected = events.filter((event) => (includeEnded || eventLifecycle(event) !== "ended") && (id === "all" || event.id === id) && (!category || event.category === category) && (!textQuery || `${event.title} ${event.description}`.toLocaleLowerCase("cs-CZ").includes(textQuery)));
  if (!selected.length) return new Response("Událost nenalezena.", { status: 404 });
  const body = academicCalendarDocument(selected, city.name);
