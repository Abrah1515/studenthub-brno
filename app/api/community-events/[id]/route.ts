...
async function managedEvent(request: Request, id: string) { const row = (await listRecords("community_events")).find((item) => String(item.id) === id); const account = await getCurrentAccount(); return row && ((account?.complete && account.accountStatus === "active" && row.author_id === account.id) || managementTokenMatches(request.headers.get("x-management-token"), row.management_token_hash)) ? row : null; }
