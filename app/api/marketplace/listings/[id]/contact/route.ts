...
export async function POST(request: Request, context: Context) {
  ...
  const saved = await insertRecord("marketplace_messages", { listing_id: id, buyer_email: parsed.data.buyerEmail.toLowerCase(), message: parsed.data.message, consent_at: new Date().toISOString(), request_fingerprint: requestFingerprint(request), delivery_status: "pending" });
  const delivery = await relayMarketplaceContact({ sellerEmail: String(listing.seller_email), buyerEmail: parsed.data.buyerEmail.toLowerCase(), title: String(listing.title), message: parsed.data.message });
