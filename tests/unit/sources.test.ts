...
describe("konektory veřejných zdrojů", () => {
  ...
  it("doplní serverové PDF globály i bez prohlížečového prostředí", async () => { const globals = globalThis as unknown as Record<string, unknown>; const original = { DOMMatrix: globals.DOMMatrix, Path2D: globals.Path2D, ImageData: globals.ImageData }; Reflect.deleteProperty(globals, "DOMMatrix"); Reflect.deleteProperty(globals, "Path2D"); Reflect.deleteProperty(globals, "ImageData"); try { const result = await parsePdf(pdfContext(await readFile("tests/fixtures/calendar-text.pdf"))); expect(result.events).toHaveLength(2); expect(typeof globals.DOMMatrix).toBe("function"); expect(typeof globals.Path2D).toBe("function"); expect(typeof globals.ImageData).toBe("function"); } finally { for (const [key, value] of Object.entries(original)) { if (value) globals[key] = value; else Reflect.deleteProperty(globals, key); } } });
