...
it("registr pokrývá a monitoruje všech 27 fakult včetně samostatných dokumentových kanálů", () => { const academic = contentSources.filter((item) => item.sourceType === "academic_calendar"); expect(academic).toHaveLength(28); expect(new Set(academic.map((item) => item.facultyId)).size).toBe(27); expect(contentSources.every((item) => item.enabled)).toBe(true); });
it("odděluje bezpečně automatické a kontrolované akademické zdroje", () => { const academic = contentSources.filter((item) => item.sourceType === "academic_calendar"); expect(academic.filter((item) => item.monitoringMode === "automatic_publish")).toHaveLength(19); expect(academic.filter((item) => item.monitoringMode === "automatic_review")).toHaveLength(9); expect(academic.filter((item) => item.monitoringMode === "not_found_monitored")).toHaveLength(0); });
