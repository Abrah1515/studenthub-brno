...
describe("víceměstský základ", () => {
  ...
  it("normalizuje města, kampusy a více měst jedné univerzity", () => { expect(migration).toContain("create table if not exists public.cities"); expect(migration).toContain("create table if not exists public.university_cities"); expect(migration).toContain("primary key (university_id, city_id)"); expect(migration).toContain("create table if not exists public.campuses"); expect(migration).toContain("create table if not exists public.offer_cities"); });
  ...
  it("generuje PWA manifest podle edice", () => { const manifest = manifestForCity(brnoCity); expect(manifest.name).toBe("StudentHub Brno"); expect(manifest.start_url).toBe("/brno"); expect(manifest.icons?.[0].src).toBe("/brand/brno/icon-192.png"); });
