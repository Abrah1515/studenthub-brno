...
describe("víceměstský základ", () => {
  ...
  it("udržuje kompatibilní veřejné assety ve shodě s novou červenou značkou", () => { expect(sha("public/icon-192.png")).toBe(sha("public/brand/brno/icon-192.png")); expect(sha("public/icon-512.png")).toBe(sha("public/brand/brno/icon-512.png")); expect(sha("public/og.png")).toBe(sha("public/brand/brno/og.png")); expect(sha("public/icon-192.png")).toBe(sha("public/brand/brno/studenthub-icon-v2-192.png")); });
  it("generuje instalovatelný PWA manifest podle edice", () => { const manifest = manifestForCity(brnoCity); expect(manifest.name).toBe("StudentHub Brno"); expect(manifest.short_name).toBe("StudentHub"); expect(manifest.start_url).toBe("/brno"); expect(manifest.scope).toBe("/"); expect(manifest.display).toBe("standalone"); expect(manifest.icons?.[0].src).toBe("/brand/brno/studenthub-icon-v2-192.png"); expect(manifest.icons?.filter((icon) => icon.purpose === "maskable")).toHaveLength(2); });
