...
describe("databázová bezpečnost", () => {
  ...
  it("bezpečně převádí historický kampusový rozsah na školu, fakultu nebo město", () => { expect(facultyCalendars).toContain("'city','university','faculty','programme','campus','national'"); expect(community).toContain("deprecated_campus_assignments"); expect(community).toContain("update public.campuses set enabled = false"); expect(community).toContain("drop constraint if exists academic_events_scope_type_check"); expect(community).not.toContain("scope_type in ('city','university','faculty','programme','campus','national')"); });
  ...
  it("nepoužívá kampusový katalog v produkční personalizaci", () => { expect(community).toContain("DEPRECATED 2026-08-04"); expect(community).toContain("revoke all on public.campuses from anon"); });
  ...
  it("používá hodinový Supabase Cron a tajemství čte pouze z Vaultu", () => { expect(scheduler).toContain("'17 * * * *'"); expect(scheduler).toContain("vault.decrypted_secrets"); expect(scheduler).toContain("studenthub_scheduler_secret"); expect(scheduler).not.toMatch(/Bearer\s+[A-Za-z0-9_-]{16,}/); });
