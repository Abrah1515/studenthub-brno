...
describe("bezpečnost komunitních akcí", () => {
  ...
  it("správa komunitní akce je výhradně profilová bez legacy tokenu", async () => { const route = await import("node:fs/promises").then(({ readFile }) => readFile("app/api/community-events/[id]/route.ts", "utf8")); expect(route).toContain("getCurrentAccount"); expect(route).toContain("row.author_id === account.id"); expect(route).not.toMatch(/management.?token|author.?email/i); });
});
