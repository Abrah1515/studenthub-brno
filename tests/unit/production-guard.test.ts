...
describe("produkční ochrana proti falešnému obsahu", () => {
  ...
  it("platná produkční konfigurace projde", () => { expect(productionConfigurationErrors({ APP_ENV: "production", NEXT_PUBLIC_SUPABASE_URL: "https://project.supabase.co", NEXT_PUBLIC_SUPABASE_ANON_KEY: "anon", SUPABASE_SERVICE_ROLE_KEY: "service", CRON_SECRET: "cron", RATE_LIMIT_SALT: "salt", DEMO_MODE: "false", ALLOW_LOCAL_FILE_STORE: "false", ALLOW_VERIFIED_FALLBACK: "false" })).toEqual([]); });
  it("administrátorská session používá databázovou roli jako jedinou autoritu", async () => { const source = await readFile("lib/admin-auth.ts", "utf8"); expect(source).toContain('.from("profiles")'); expect(source).toContain("profile?.role"); expect(source).not.toMatch(/app_metadata|claimedRole/); expect(source).toContain("profile.city_id"); expect(source).toContain("profile.faculty_id"); });
  ...
  it("běžný uživatel i administrátor používají jediný callback a obnovovací endpoint", async () => { const [recovery, callback, adminForm] = await Promise.all([readFile("app/api/auth/recover/route.ts", "utf8"), readFile("app/auth/callback/route.ts", "utf8"), readFile("components/admin-login-form.tsx", "utf8")]); expect(recovery).toContain("/auth/callback?next="); expect(recovery).toContain("/ucet/obnova"); expect(callback).toContain("exchangeCodeForSession"); expect(adminForm).toContain('/api/auth/recover'); await expect(readFile("app/api/admin/recover/route.ts", "utf8")).rejects.toThrow(); });
});
