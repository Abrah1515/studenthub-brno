...
describe("produkční ochrana proti falešnému obsahu", () => {
  ...
  it("obnova administrátora zachová fragment session až do nastavení hesla", async () => { const [form, invite] = await Promise.all([readFile("components/admin-password-form.tsx", "utf8"), readFile("scripts/invite-superadmin.mjs", "utf8")]); expect(form).toContain("detectSessionInUrl: false"); expect(form).toContain('fragment.get("access_token")'); expect(form).toContain('fragment.get("refresh_token")'); expect(form).toContain("client.auth.setSession"); expect(form).toContain("window.history.replaceState"); expect(invite).toContain('redirectTo: `${siteUrl}/admin/obnova`'); expect(invite).not.toContain("auth/callback?next=/admin/obnova"); });
});
