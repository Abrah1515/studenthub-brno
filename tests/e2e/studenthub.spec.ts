...
async function dismissOverlays(page: import("@playwright/test").Page) {
  const consent = page.getByTestId("cookie-consent"); if (await consent.isVisible()) { await consent.getByRole("button", { name: "Pouze nezbytné" }).click(); await expect(consent).toBeHidden(); }
...
test("cookies a povinný onboarding nikdy nevytvoří dva modály současně", async ({ page }) => { await page.evaluate(() => { sessionStorage.setItem("studenthub-e2e-overlays", "manual"); localStorage.clear(); }); await page.reload(); const modals = page.locator('[role="dialog"][aria-modal="true"]'); await expect(modals).toHaveCount(1); await expect(page.getByTestId("cookie-consent")).toBeVisible(); await expect(page.getByRole("button", { name: "Pouze nezbytné" })).toBeFocused(); expect(await page.evaluate(() => document.querySelector(".app-shell")?.hasAttribute("inert"))).toBe(true); await page.keyboard.press("Escape"); const picker = page.getByTestId("first-run-picker"); await expect(picker).toBeVisible(); await expect(modals).toHaveCount(1); await expect(picker.getByLabel("Moje město")).toBeFocused(); await page.keyboard.press("Escape"); await expect(picker).toBeVisible(); await expect(modals).toHaveCount(1); await picker.getByRole("button", { name: "Pokračovat vědomě bez výběru školy pro celé město Brno" }).click(); await expect(modals).toHaveCount(0); });
