...
async function openDirectoryFilters(page: import("@playwright/test").Page) {
  ...
  return page.locator("#hlavni-obsah .responsive-filter-desktop").first();
}
...
test("filtruje ověřené akademické události", async ({ page }) => {
  ...
  await filters.getByLabel("Kategorie").selectOption("Zkouškové období");
  if (await filters.getAttribute("role") === "dialog") await filters.getByRole("button", { name: /Zobrazit \d+ událostí/ }).click();
  await expect(page.locator(".result-count:visible").filter({ hasText: /ověřených událostí/ }).first()).toBeVisible();
...
test("komunitní kalendář publikuje přes jednotný účet bez e-mailu ve formuláři", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "desktop-1440");
  const title = `E2E komunitní akce ${Date.now()}`; let submitted = "";
  await page.route("**/api/auth/me", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ user: { id: "11111111-1111-4111-8111-111111111111" }, profile: { complete: true, trustedEventPublisher: true } }) }));
  await page.route("**/api/community-events", async (route) => { if (route.request().method() !== "POST") return route.continue(); submitted = route.request().postData() || ""; return route.fulfill({ status: 201, contentType: "application/json", body: JSON.stringify({ item: { id: "21111111-1111-4111-8111-111111111111", cityId: "brno", title, category: "Kultura", startsAt: new Date(Date.now()+2*86_400_000).toISOString(), venue: "Veřejná knihovna v Brně", description: "Veřejná studentská akce vytvořená koncovým regresním testem.", isFree: true, currency: "CZK", sourceType: "community", createdAt: new Date().toISOString(), interestCount: 0, interestLast24h: 0, author: { username: "audit_student", displayName: "Audit", verifiedEmail: true, legacy: false } }, published: true, status: "published", message: "Akce je zveřejněná a propojená s vaším profilem." }) }); });
  await page.goto("/brno/kalendar?view=community"); await expect(page.getByRole("tab", { name: "Co se děje" })).toHaveAttribute("aria-selected", "true"); await expect(page.getByText(/Rozlišujeme veřejné zdroje a komunitní příspěvky/).first()).toBeVisible();
...
test("filtruje místa a vypnuté nabídky přesměrují bez prázdného bloku", async ({ page }) => { await page.goto("/brno/mista"); const filters = await openDirectoryFilters(page); await filters.getByLabel("Kategorie").selectOption("Knihovna"); await expect(page.getByText("Ústřední knihovna VUT")).toBeVisible(); await expect(page.getByText("DEMO")).toHaveCount(0); await page.goto("/brno/nabidky?q=neexistujici-nabidka"); await expect(page).toHaveURL(/\/brno$/); await expect(page.getByRole("heading", { name: "Aktuální nabídky" })).toHaveCount(0); await expect(page.getByRole("navigation", { name: "Hlavní navigace" }).getByRole("link", { name: /Nabídky/ })).toHaveCount(0); });
...
test("každé otevření kalendáře obnoví Moji fakultu, dočasný filtr ji nepřepíše", async ({ page }, testInfo) => {
  ...
  await page.goto("/brno/kalendar?university=muni&faculty=muni-fi"); const filters = await openDirectoryFilters(page);
  await expect(filters.getByLabel("Fakulta", { exact: true })).toHaveValue("muni-fi");
  await expect(filters.getByLabel("Ročník", { exact: true })).toHaveValue("");
  await filters.getByRole("button", { name: "Moje nastavení" }).click();
  await expect(page).toHaveURL(/university=vut.*faculty=vut-fekt/);
  await expect(page).toHaveURL(/year=2/);
  await expect(filters.getByLabel("Ročník", { exact: true })).toHaveValue("2");
  await filters.getByLabel("Univerzita").selectOption("muni");
  await filters.getByLabel("Fakulta", { exact: true }).selectOption("muni-fi");
  await filters.getByLabel("Ročník", { exact: true }).selectOption("3");
  if (testInfo.project.name !== "desktop-1440") await filters.getByRole("button", { name: /Zobrazit \d+ událostí/ }).click();
...
test("odstraněná sekce Pro spolky přesměruje na Kontakt", async ({ page }, testInfo) => { test.skip(testInfo.project.name !== "desktop-1440"); await page.goto("/navrhnout-obsah"); await expect(page).toHaveURL(/\/kontakt$/); await expect(page.getByRole("heading", { name: "Kontakt", exact: true })).toBeVisible(); await expect(page.getByText("Pro spolky", { exact: true })).toHaveCount(0); });

test("všechny veřejné formuláře odmítnou nevalidní data na serveru", async ({ request }, testInfo) => { test.skip(testInfo.project.name !== "desktop-1440"); for (const endpoint of ["/api/jobs", "/api/contact"]) { const response = await request.post(endpoint, { data: { company: "spam" } }); expect(response.status(), endpoint).toBe(422); } expect((await request.post("/api/submissions", { data: { company: "spam" } })).status()).toBe(404); expect((await request.post("/api/service-requests", { data: {} })).status()).toBe(410); const event = await request.post("/api/community-events", { multipart: { company: "spam" } }); expect([401,503]).toContain(event.status()); });
...
test("honeypot formuláře není viditelný ani dostupný čtečce", async ({ page }, testInfo) => {
  ...
  expect(await honeypot.evaluate((element) => getComputedStyle(element).position)).toBe("absolute");
});

test("staré URL přesměrují a školní stránky uvádějí nezávislost", async ({ page }) => { await page.goto("/kalendar"); await expect(page).toHaveURL(/\/brno\/kalendar$/); for (const school of ["muni", "vut", "mendelu"]) { await page.goto(`/${school}`); await expect(page).toHaveURL(new RegExp(`/brno/skoly/${school}$`)); const main = page.locator("#hlavni-obsah"); await expect(main.getByText(/Nezávislý projekt:/)).toBeVisible(); await expect(main.getByText(/nenahrazuje školní informační systém/)).toBeVisible(); } });
...
test("motiv podporuje systém, světlo a tmu s perzistencí", async ({ page }, testInfo) => { let group = await openThemeSettings(page, testInfo.project.name); await group.getByRole("radio", { name: "Tmavý režim" }).click(); await expect(page.locator("html")).toHaveAttribute("data-theme", "dark"); await page.reload(); await dismissOverlays(page); group = await openThemeSettings(page, testInfo.project.name); await expect(group.getByRole("radio", { name: "Tmavý režim" })).toHaveAttribute("aria-checked", "true"); await group.getByRole("radio", { name: "Podle zařízení" }).click(); expect(await page.evaluate(() => localStorage.getItem("studenthub-theme"))).toBe("system"); });
...
test("kritické stránky nepřetékají na 360 px ani na širokém desktopu", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "desktop-1440");
  for (const viewport of [{ width: 360, height: 800, label: "phone-360" }, { width: 1728, height: 1000, label: "desktop-wide" }]) {
    await page.setViewportSize(viewport);
    for (const path of ["/brno", "/brno/kalendar", "/brno/mista", "/hlidac", "/brno/burza", "/partak"]) {
      await page.goto(path, { waitUntil: "domcontentloaded" });
      expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth), `${viewport.label} ${path}`).toBeLessThanOrEqual(1);
      expect(await page.locator("#hlavni-obsah").isVisible(), `${viewport.label} ${path}`).toBe(true);
    }
    await page.goto("/brno");
    await page.screenshot({ path: `artifacts/audit-${viewport.label}.png`, fullPage: true });
  }
});

test("administrace na všech breakpointech používá jen Supabase přihlášení a neprotéká", async ({ page }) => { const response=await page.goto("/admin"); expect(response?.status()).toBeLessThan(400); await expect(page).toHaveURL(/\/admin\/prihlaseni/); await expect(page.getByRole("heading", { name: "Administrace" })).toBeVisible(); await expect(page.getByLabel("E-mail")).toBeVisible(); await expect(page.getByLabel("Heslo")).toBeVisible(); await expect(page.getByRole("button", { name: /Google/i })).toHaveCount(0); expect(await page.evaluate(() => document.documentElement.scrollWidth-document.documentElement.clientWidth)).toBeLessThanOrEqual(1); });
...
test("nepřihlášený uživatel nemůže otevřít administrační frontu změn", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "desktop-1440");
