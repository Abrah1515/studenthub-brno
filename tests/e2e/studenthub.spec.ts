...
test("validuje a uloží poptávku technické pomoci", async ({ page }, testInfo) => { test.skip(testInfo.project.name !== "desktop-1440"); await page.goto("/pomoc"); await page.getByRole("button", { name: "Odeslat poptávku" }).click(); await expect(page.getByText("Uveďte prosím jméno.")).toBeVisible(); const form = page.getByTestId("service-request-form"); await form.getByLabel("Veřejný název žádosti *").fill("Pomoc s vypínáním notebooku"); await form.getByLabel("Veřejná přezdívka *").fill("Student E2E"); await form.getByLabel("Jméno *").fill("Testovací Student"); await form.locator('input[name="email"]').fill("student@example.cz"); await form.getByLabel("Popis problému *").fill("Notebook se při startu vypíná a potřebuji bezpečně zkontrolovat hardware."); await form.getByLabel("Přibližná lokalita *").fill("Brno-střed"); const futureDate = await page.evaluate(() => new Date(Date.now() + 7 * 86_400_000).toISOString().slice(0, 10)); await form.getByLabel("Preferovaný termín *").fill(futureDate); await form.getByText(/Souhlasím se zpracováním uvedených údajů/).click(); await form.getByText(/Souhlasím s okamžitým zveřejněním přezdívky/).click(); await form.getByRole("button", { name: "Odeslat poptávku" }).click(); await expect(page.getByTestId("request-success")).toBeVisible(); await expect(page.getByRole("heading", { name: "Žádost je zveřejněná" })).toBeVisible(); });
...
test("kritické stránky nepřetékají na 360 px ani na širokém desktopu", async ({ page }, testInfo) => {
  ...
  for (const viewport of [{ width: 360, height: 800, label: "phone-360" }, { width: 1728, height: 1000, label: "desktop-wide" }]) {
    ...
    for (const path of ["/brno", "/brno/kalendar", "/brno/mista", "/hlidac", "/pomoc", "/partak"]) {
...
test("PWA manifest, ikony a service worker jsou dostupné a necachují dynamické HTML", async ({ page, request }, testInfo) => {
  ...
  expect(cacheAudit.keys).toEqual(["studenthub-static-v4"]); expect(cacheAudit.dynamic).toBe(false); expect(cacheAudit.admin).toBe(false); expect(cacheAudit.api).toBe(false);
});
