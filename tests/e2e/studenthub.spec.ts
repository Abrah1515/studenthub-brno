...
test("všechny veřejné formuláře odmítnou nevalidní JSON na serveru", async ({ request }, testInfo) => { test.skip(testInfo.project.name !== "desktop-1440"); for (const endpoint of ["/api/service-requests", "/api/jobs", "/api/submissions", "/api/contact"]) { const response = await request.post(endpoint, { data: { company: "spam" } }); expect(response.status(), endpoint).toBe(422); } });
