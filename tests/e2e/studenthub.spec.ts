...
test("přihlášená administrace používá hluboké odkazy, historii, živá hlášení a nepřetéká", async ({ page }, testInfo) => { await page.goto("/admin/prihlaseni"); await page.getByLabel("E-mail").fill("audit@example.cz"); await page.getByLabel("Heslo").fill("local-test-password-2026"); await page.getByRole("button", { name: "Přihlásit se" }).click(); await page.waitForURL("**/admin"); await expect(page.getByText("Lokální ověřený fallback.")).toBeVisible(); await expect(page.getByText("28 zdrojů")).toBeVisible(); if (testInfo.project.name === "desktop-1440") await page.getByRole("navigation", { name: "Sekce administrace" }).getByRole("link", { name: "Živý stav míst" }).click(); else await page.getByLabel("Aktivní sekce").selectOption("live_reports"); await expect(page).toHaveURL(/section=live_reports/); await expect(page.getByRole("heading", { name: "Živý stav míst", exact: true })).toBeVisible(); expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).toBeLessThanOrEqual(1); if (testInfo.project.name === "desktop-1440") await page.getByRole("navigation", { name: "Sekce administrace" }).getByRole("link", { name: "Návštěvnost" }).click(); else await page.getByLabel("Aktivní sekce").selectOption("analytics"); await expect(page).toHaveURL(/section=analytics/); await expect(page.getByRole("heading", { name: "Návštěvnost se souhlasem", exact: true })).toBeVisible(); if (testInfo.project.name === "desktop-1440") await page.getByRole("navigation", { name: "Sekce administrace" }).getByRole("link", { name: "Kontaktní zprávy" }).click(); else await page.getByLabel("Aktivní sekce").selectOption("contact_messages"); await expect(page.getByRole("heading", { name: "Kontaktní zprávy", exact: true })).toBeVisible(); await page.goBack(); await expect(page).toHaveURL(/section=analytics/); await expect(page.getByRole("heading", { name: "Návštěvnost se souhlasem", exact: true })).toBeVisible(); expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).toBeLessThanOrEqual(1); await page.screenshot({ path: `artifacts/admin-${testInfo.project.name}.png`, fullPage: true }); await page.getByRole("button", { name: "Odhlásit" }).click(); await expect(page).toHaveURL(/\/admin\/prihlaseni/); });
