...
test("všechny veřejné formuláře odmítnou nevalidní data na serveru", async ({ request }, testInfo) => { test.skip(testInfo.project.name !== "desktop-1440"); for (const endpoint of ["/api/jobs", "/api/submissions", "/api/contact"]) { const response = await request.post(endpoint, { data: { company: "spam" } }); expect(response.status(), endpoint).toBe(422); } expect((await request.post("/api/service-requests", { data: {} })).status()).toBe(410); const event = await request.post("/api/community-events", { multipart: { company: "spam" } }); expect([401,503]).toContain(event.status()); });
