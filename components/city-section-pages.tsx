...
export async function CityCalendarPage({ city, selection = {} }: { city: City; selection?: { universityId?: string; facultyId?: string; studyYear?: StudyYear } }) { const events = await getAcademicEvents(city.id); return <div className="page-stack"><PageHeading eyebrow={`Vysoké školy · ${city.name}`} title="Akademický kalendář" description="Termíny přebíráme pouze z veřejných zdrojů. U každého záznamu uvádíme původ a poslední ověření; StudentHub nenahrazuje školní informační systém." /><EventExplorer events={events} cityId={city.id} initialUniversityId={selection.universityId} initialFacultyId={selection.facultyId} initialStudyYear={selection.studyYear} /></div>; }
