...
export async function SchoolHub({ universityId, city }: { universityId: string; city: City }) {
  ...
  return <div className="page-stack school-hub">
    ...
    <section className="school-summary"><article><CalendarDays size={19} /><strong>{schoolEvents.length}</strong><span>ověřené termíny</span></article><article><Sparkles size={19} /><strong>{schoolOffers.length}</strong><span>relevantní nabídky</span></article><article><MapPin size={19} /><strong>{schoolPlaces.length}</strong><span>místa poblíž kampusů</span></article><article><BriefcaseBusiness size={19} /><strong>{schoolJobs.length}</strong><span>vybrané brigády</span></article></section>
    <div className="school-grid"><section className="dashboard-section"><div className="section-head"><div><span className="section-icon"><CalendarDays size={17} /></span><h2>Veřejné termíny a akce</h2></div><Link href={`${base}/kalendar?university=${universityId}`}>Všechny <ArrowRight size={16} /></Link></div><div className="school-list">{schoolEvents.length ? schoolEvents.map((event) => <Link href={`${base}/kalendar#${event.id}`} key={event.id}><time>{formatShortDate(event.start)}</time><div><strong>{event.title}</strong><small>{event.source}</small></div></Link>) : <p className="muted">Zatím nemáme ověřené budoucí termíny.</p>}</div></section><section className="dashboard-section"><div className="section-head"><div><span className="section-icon"><Sparkles size={17} /></span><h2>Nabídky pro studenty</h2></div><Link href={`${base}/nabidky?university=${universityId}`}>Všechny <ArrowRight size={16} /></Link></div><div className="school-list">{schoolOffers.length ? schoolOffers.map((offer) => <Link href={`${base}/nabidky#${offer.id}`} key={offer.id}><time>{offer.discount}</time><div><strong>{offer.title}</strong><small>{offer.partner}{offer.campus ? ` · ${offer.campus}` : ""}</small></div></Link>) : <p className="muted">Zatím nemáme ověřené partnerské nabídky.</p>}</div></section></div>
    <section className="campus-section"><div><span className="eyebrow">Podle kampusu</span><h2>Užitečná místa v dosahu</h2></div><div className="campus-grid">{university.campuses.map((campus) => <article key={campus.name}><MapPin size={18} /><strong>{campus.name}</strong><span>{schoolPlaces.filter((place) => place.campus === campus.name || !place.campus).length} ověřených míst</span><Link href={`${base}/mista?university=${universityId}&campus=${encodeURIComponent(campus.name)}`}>Zobrazit na mapě <ArrowRight size={15} /></Link></article>)}</div></section>
