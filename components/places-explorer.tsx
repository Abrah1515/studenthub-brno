...
export function PlacesExplorer({ items, city }: { items: Place[]; city: City }) {
  ...
  const filtered = useMemo(() => items.filter((place) => (category === "Všechny" || place.category === category) && (!universityId || !place.universityIds?.length || place.universityIds.includes(universityId)) && (!facultyId || !place.facultyIds?.length || place.facultyIds.includes(facultyId)) && includesFolded(`${place.name} ${place.address} ${place.note}`, query)), [items, query, category, universityId, facultyId]);
  const categories = ["Všechny", ...new Set(items.map((place) => place.category))];
  return (
    <>
      <section className="filter-panel places-filters" aria-label="Filtry míst"><label className="search-field"><span>Hledat místo nebo adresu</span><div><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Např. studovna…" /></div></label><label><span>Kategorie</span><div className="select-wrap"><select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select><ChevronDown size={16} /></div></label><label><span>Univerzita</span><select aria-label="Univerzita" value={universityId} onChange={(event) => { setSchoolFilter(event.target.value); setFacultyFilter(""); }}><option value="">Všechny školy</option>{universities.map((item) => <option key={item.id} value={item.id}>{item.shortName}</option>)}</select></label><label><span>Fakulta</span><select value={facultyId} onChange={(event) => setFacultyFilter(event.target.value)} disabled={!universityId}><option value="">Všechny fakulty</option>{facultiesFor(universityId).map((item) => <option key={item.id} value={item.id}>{item.shortName}</option>)}</select></label><div className="filter-actions"><button className="button button-secondary" type="button" onClick={() => { setQuery(""); setCategory("Všechny"); setSchoolFilter(""); setFacultyFilter(""); }}><RotateCcw size={16} />Resetovat filtry</button></div></section>
      <section className="places-layout">
        <div className="places-list"><div className="result-count"><strong>{filtered.length}</strong> míst v seznamu</div>{filtered.length === 0 ? <div className="empty-state"><MapPin size={26} /><h2>Žádné místo neodpovídá filtrům</h2><p>Aktivní filtry: {[query, category !== "Všechny" ? category : "", universityId, facultyId].filter(Boolean).join(" · ") || "žádné"}.</p><button className="button button-secondary" onClick={() => { setQuery(""); setCategory("Všechny"); setSchoolFilter(""); setFacultyFilter(""); }}>Resetovat filtry</button></div> : filtered.map((place) => <article id={place.id} key={place.id} className={`place-card ${selected === place.id ? "selected" : ""}`}><button className="place-card-main" type="button" onClick={() => setSelected(selected === place.id ? null : place.id)} aria-expanded={selected === place.id}><span className="place-icon"><MapPin size={19} /></span><span><span className="result-labels"><i className="tag">{place.category}</i></span><strong>{place.name}</strong><small>{place.address}</small></span><LocateFixed size={17} /></button>{selected === place.id && <div className="place-details"><p>{place.whyVisit || place.note}</p><dl><div><dt>Otevírací doba</dt><dd>{place.hours || "Ověřte na webu provozovatele"}</dd></div><div><dt>Ověřeno</dt><dd>{formatPragueTimestamp(place.openingHoursVerifiedAt || place.lastVerifiedAt)}</dd></div>{place.priceLevel && <div><dt>Cenová úroveň</dt><dd>{place.priceLevel === "free" ? "zdarma" : place.priceLevel === "low" ? "nízká" : "liší se"}</dd></div>}{place.studentDiscount && <div><dt>Studentská podmínka</dt><dd>{place.studentDiscount}</dd></div>}</dl><div><a href={place.website} target="_blank" rel="noopener noreferrer" className="button button-secondary"><ExternalLink size={16} />Web</a><a href={`https://www.openstreetmap.org/directions?to=${place.lat}%2C${place.lng}`} target="_blank" rel="noopener noreferrer" className="button button-primary"><Navigation size={16} />Navigovat</a><a href={`/kontakt?subject=${encodeURIComponent(`Oprava místa: ${place.name}`)}`} className="text-link">Nahlásit změnu</a></div></div>}</article>)}</div>
