...
function PlacesMap({ items, city, userLocation, selectedId, onSelect }: { items: Place[]; city: City; userLocation: ClientLocation | null; selectedId: string | null; onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      if (!mapRef.current) {
        mapRef.current = L.map(element.current, { zoomControl: false, scrollWheelZoom: false, maxBounds: city.mapBounds }).setView([city.latitude, city.longitude], city.mapZoom);
...
export function PlacesExplorer({ items, city }: { items: Place[]; city: City }) {
  ...
  function resetFilters() {
    setQuery(""); setCategory(allCategories); setSchoolFilter(""); setFacultyFilter(""); setAvailableOnly(false); setSortMode("default");
  ...
  return <>
    ...
    <section id="places-filters" className={`filter-panel places-filters collapsible-filter-panel ${filtersOpen ? "is-open" : ""}`} aria-label="Filtry míst"><label className="search-field"><span>Hledat místo nebo adresu</span><div><Search size={17} /><input value={query} onChange={(event) => { setQuery(event.target.value); replaceQuery({ q: event.target.value.trim() || undefined, campus: undefined, filters: undefined }); }} placeholder="Např. studovna…" /></div></label><label><span>Kategorie</span><div className="select-wrap"><select value={category} onChange={(event) => { setCategory(event.target.value); replaceQuery({ category: event.target.value === allCategories ? undefined : event.target.value, filters: undefined }); }}>{categories.map((item) => <option key={item}>{item}</option>)}</select><ChevronDown size={16} /></div></label><label><span>Univerzita</span><select aria-label="Univerzita" value={universityId} onChange={(event) => { setSchoolFilter(event.target.value); setFacultyFilter(""); replaceQuery({ university: event.target.value || undefined, faculty: undefined, filters: undefined }); }}><option value="">Všechny školy</option>{universities.map((item) => <option key={item.id} value={item.id}>{item.shortName}</option>)}</select></label><label><span>Fakulta</span><select value={facultyId} onChange={(event) => { setFacultyFilter(event.target.value); replaceQuery({ faculty: event.target.value || undefined, filters: undefined }); }} disabled={!universityId}><option value="">Všechny fakulty</option>{facultiesFor(universityId).map((item) => <option key={item.id} value={item.id}>{item.shortName}</option>)}</select></label><label className="checkbox-row"><input type="checkbox" checked={availableOnly} onChange={(event) => setAvailableOnly(event.target.checked)} /><span>Aktuálně dostupné</span></label><label><span>Řazení</span><select value={sortMode} onChange={(event) => setSortMode(event.target.value as "default" | "distance")}><option value="default">Podle názvu</option><option value="distance" disabled={!userLocation}>Podle vzdálenosti</option></select></label><div className="filter-actions"><button className="button button-secondary" type="button" onClick={resetFilters}><RotateCcw size={16} />Resetovat filtry</button></div></section>
    <div className="location-toolbar"><button type="button" className="button button-secondary" onClick={requestLocation} disabled={locationStatus === "loading"}><LocateFixed size={17} />{locationStatus === "loading" ? "Zjišťuji polohu…" : userLocation ? "Aktualizovat moji polohu" : "Použít moji polohu"}</button>{userLocation && filtered[0] && <span className="nearest-place">Nejblíž: <strong>{filtered[0].name}</strong></span>}{locationMessage && <p className={locationStatus === "error" ? "location-error" : "location-note"} role="status">{locationMessage}</p>}</div>
    <section className="places-layout">
      <div className="places-list"><div className="result-count"><strong>{filtered.length}</strong> míst v seznamu</div>{filtered.length === 0 ? <div className="empty-state"><MapPin size={26} /><h2>Žádné místo neodpovídá filtrům</h2><p>Aktivní filtry: {[query, category !== allCategories ? category : "", universityId, facultyId, availableOnly ? "aktuálně dostupné" : ""].filter(Boolean).join(" · ") || "žádné"}.</p><button className="button button-secondary" onClick={resetFilters}>Resetovat filtry</button></div> : filtered.map((place) => { const distance = userLocation ? haversineDistanceKm(userLocation, { lat: place.lat, lng: place.lng }) : null; const proximityBand = distance !== null && distance <= 1 ? "near" as const : "unknown" as const; return <article ref={(node) => { if (node) cardRefs.current.set(place.id, node); else cardRefs.current.delete(place.id); }} id={place.id} key={place.id} className={`place-card ${selected === place.id ? "selected" : ""}`}><button className="place-card-main" type="button" onClick={() => setSelected(selected === place.id ? null : place.id)} aria-expanded={selected === place.id}><span className="place-icon"><MapPin size={19} /></span><span><span className="result-labels"><i className="tag">{place.category}</i>{liveSummaries[place.id]?.code !== "unknown" && <i className={`tag live-${liveSummaries[place.id]?.code}`}>{liveSummaries[place.id]?.label}</i>}</span><strong>{place.name}</strong><small>{place.address}</small>{distance !== null && <small className="place-distance">{distance < 1 ? `${Math.round(distance * 1000)} m` : `${distance.toFixed(1)} km`} od vás</small>}</span><LocateFixed size={17} /></button>{selected === place.id && <div className="place-details"><p>{place.whyVisit || place.note}</p><dl>{(place.accessConditions || place.studentDiscount) && <div><dt>Podmínky přístupu</dt><dd>{place.accessConditions || place.studentDiscount}</dd></div>}<div><dt>Otevírací doba</dt><dd>{place.hours || "Ověřte na webu provozovatele"}</dd></div><div><dt>Ověřeno</dt><dd>{formatPragueTimestamp(place.openingHoursVerifiedAt || place.lastVerifiedAt)}{place.sourceSyncStatus && place.sourceSyncStatus !== "verified" ? " · změna čeká na kontrolu" : ""}</dd></div>{place.priceLevel && <div><dt>Cenová úroveň</dt><dd>{place.priceLevel === "free" ? "zdarma" : place.priceLevel === "low" ? "nízká" : "liší se"}</dd></div>}</dl><PlaceLiveStatus placeId={place.id} category={place.category} initialSummary={liveSummaries[place.id]} proximityBand={proximityBand} onChange={(summary) => setLiveSummaries((current) => ({ ...current, [place.id]: summary }))} /><div><a href={place.website} target="_blank" rel="noopener noreferrer" className="button button-secondary"><ExternalLink size={16} />Web</a><a href={googleMapsDirectionsUrl(place)} target="_blank" rel="noopener noreferrer" className="button button-primary"><Navigation size={16} />Navigovat</a><a href={`/kontakt?subject=${encodeURIComponent(`Oprava místa: ${place.name}`)}`} className="text-link">Nahlásit změnu</a></div></div>}</article>; })}</div>
      <div className="map-shell"><PlacesMap items={filtered} city={city} userLocation={userLocation} selectedId={mapPreview || selected} onSelect={selectFromMap} /><div className="map-caption"><span><MapPin size={16} />{city.name} a okolí</span><small>Mapová data © OpenStreetMap</small></div></div>
    </section>
  </>;
}
