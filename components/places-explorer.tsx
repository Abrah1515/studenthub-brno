...
import { Armchair, Beer, BookOpen, BriefcaseBusiness, ChevronDown, Coffee, Droplets, Dumbbell, ExternalLink, HeartHandshake, Library, LocateFixed, MapPin, Navigation, PlusCircle, RotateCcw, Search, Sandwich, Toilet, Trees, Utensils, Wrench } from "lucide-react";
...
const categorySymbols: Record<PlaceCategoryCode,string>={restaurant:"R",cafe:"K",pub_bar:"B",fast_food:"F",canteen:"M",library:"L",study_room:"S",coworking:"C",public_toilet:"WC",drinking_fountain:"H₂O",sport:"P",student_service:"i",counselling:"+",park:"🌳",bench:"—",other:"•"};
...
export function PlacesExplorer({ items, city }: { items: Place[]; city: City }) {
  ...
  const filtered = useMemo(() => uniqueItems.filter((place) => { const haystack = `${place.name} ${place.address} ${place.note} ${place.whyVisit || ""}`; const campusStem = campusQuery.length > 4 ? campusQuery.slice(0, -2) : campusQuery; return (showBenches||categoryCode(place)!=="bench")&&(category === allCategories || place.category === category) && (!universityId || !place.universityIds?.length || place.universityIds.includes(universityId)) && (!facultyId || !place.facultyIds?.length || place.facultyIds.includes(facultyId)) && (!availableOnly || liveSummaries[place.id]?.available) && (!openNowOnly||isPlaceOpenNow(place.hours)) && (!priceFilter||place.priceLevel===priceFilter) && (!studyOnly||place.studySuitable===true) && (!wifiOnly||place.wifiAvailable===true) && (!outletsOnly||place.outletsAvailable===true) && (!publicOnly||place.publicAccess===true)&&(!studentOnly||place.studentOnly===true)&&(!accessibleOnly||place.accessibility==="accessible")&&(originFilter==="all"||(place.origin||"official")===originFilter) && (includesFolded(haystack, query) || Boolean(campusQuery && campusStem && includesFolded(haystack, campusStem))); }).sort((a, b) => sortMode === "distance" && userLocation ? haversineDistanceKm(userLocation, { lat: a.lat, lng: a.lng }) - haversineDistanceKm(userLocation, { lat: b.lat, lng: b.lng }) : a.name.localeCompare(b.name, "cs-CZ")), [uniqueItems, query, campusQuery, category, universityId, facultyId, availableOnly,openNowOnly,priceFilter,studyOnly,wifiOnly,outletsOnly,publicOnly,studentOnly,accessibleOnly,showBenches,originFilter, liveSummaries, sortMode, userLocation]);
  const categories = [allCategories,...placeCategoryCodes.map((code)=>placeCategoryLabels[code])];
  const activeFilterCount = [query.trim(), category !== allCategories, universityId, facultyId, availableOnly,openNowOnly,priceFilter,studyOnly,wifiOnly,outletsOnly,publicOnly,studentOnly,accessibleOnly,showBenches,originFilter!=="all", sortMode !== "default"].filter(Boolean).length;
  ...
  function resetFilters() {
    setQuery(""); setCategory(allCategories); setSchoolFilter(""); setFacultyFilter(""); setAvailableOnly(false);setOpenNowOnly(false);setPriceFilter("");setStudyOnly(false);setWifiOnly(false);setOutletsOnly(false);setPublicOnly(false);setStudentOnly(false);setAccessibleOnly(false);setShowBenches(false);setOriginFilter("all"); setSortMode("default");
  ...
  const filterControls = <><label className="search-field"><span>Hledat místo nebo adresu</span><div><Search size={17}/><input value={query} onChange={(event)=>{setQuery(event.target.value);replaceQuery({q:event.target.value.trim()||undefined,campus:undefined,filters:undefined})}} placeholder="Např. knihovna, Veveří…"/></div></label><label><span>Kategorie</span><div className="select-wrap"><select value={category} onChange={(event)=>{setCategory(event.target.value);replaceQuery({category:event.target.value===allCategories?undefined:event.target.value,filters:undefined});if(event.target.value==="Lavičky a odpočinek")setShowBenches(true)}}>{categories.map((item)=><option key={item}>{item}</option>)}</select><ChevronDown size={16}/></div></label><label><span>Univerzita</span><select aria-label="Univerzita" value={universityId} onChange={(event)=>{setSchoolFilter(event.target.value);setFacultyFilter("");replaceQuery({university:event.target.value||undefined,faculty:undefined,filters:undefined})}}><option value="">Všechny školy</option>{universities.map((item)=><option key={item.id} value={item.id}>{item.shortName}</option>)}</select></label><label><span>Fakulta</span><select value={facultyId} onChange={(event)=>{setFacultyFilter(event.target.value);replaceQuery({faculty:event.target.value||undefined,filters:undefined})}} disabled={!universityId}><option value="">Všechny fakulty</option>{facultiesFor(universityId).map((item)=><option key={item.id} value={item.id}>{item.shortName}</option>)}</select></label><label><span>Cena</span><select value={priceFilter} onChange={(event)=>setPriceFilter(event.target.value)}><option value="">Všechny ceny</option><option value="free">Zdarma</option><option value="low">Nízká</option><option value="medium">Střední</option><option value="high">Vyšší</option><option value="varies">Liší se</option></select></label><label><span>Původ záznamu</span><select value={originFilter} onChange={(event)=>setOriginFilter(event.target.value as typeof originFilter)}><option value="all">Všechny zdroje</option><option value="official">Ověřený veřejný zdroj</option><option value="community">Licencovaný komunitní zdroj / návrh</option></select></label><label><span>Řazení</span><select value={sortMode} onChange={(event)=>setSortMode(event.target.value as "default"|"distance")}><option value="default">Podle názvu</option><option value="distance" disabled={!userLocation}>Podle vzdálenosti</option></select></label><div className="place-quick-filters"><label><input type="checkbox" checked={openNowOnly} onChange={(event)=>setOpenNowOnly(event.target.checked)}/>Aktuálně otevřeno</label><label><input type="checkbox" checked={availableOnly} onChange={(event)=>setAvailableOnly(event.target.checked)}/>Aktuálně dostupné</label><label><input type="checkbox" checked={publicOnly} onChange={(event)=>setPublicOnly(event.target.checked)}/>Veřejně přístupné</label><label><input type="checkbox" checked={studentOnly} onChange={(event)=>setStudentOnly(event.target.checked)}/>Pouze pro studenty</label><label><input type="checkbox" checked={accessibleOnly} onChange={(event)=>setAccessibleOnly(event.target.checked)}/>Bezbariérové</label><label><input type="checkbox" checked={showBenches} onChange={(event)=>setShowBenches(event.target.checked)}/>Vrstva laviček</label><label><input type="checkbox" checked={studyOnly} onChange={(event)=>setStudyOnly(event.target.checked)}/>Vhodné ke studiu</label><label><input type="checkbox" checked={wifiOnly} onChange={(event)=>setWifiOnly(event.target.checked)}/>Wi‑Fi</label><label><input type="checkbox" checked={outletsOnly} onChange={(event)=>setOutletsOnly(event.target.checked)}/>Zásuvky</label></div><div className="filter-actions"><button className="button button-secondary" type="button" onClick={resetFilters}><RotateCcw size={16}/>Resetovat filtry</button></div></>;
  return <>
