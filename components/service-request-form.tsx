...
export function ServiceRequestForm() {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<ServiceRequestInput>({
    ...
    defaultValues: { publicTitle: "", name: "", email: "", phone: "", serviceType: "windows-linux", description: "", location: "", preferredDate: "", consent: false, publishConsent: false, company: "" },
  });
  ...
  return (
    <form className="form-card request-form" onSubmit={handleSubmit(submit)} noValidate data-testid="service-request-form">
      ...
      <label className="checkbox-field"><input {...register("publishConsent")} type="checkbox" /><span>Souhlasím, aby se po schválení zobrazil název, popis, typ pomoci a přibližná lokalita. Jméno, e-mail ani telefon se nezveřejní. *</span></label>
