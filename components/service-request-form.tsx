...
export function ServiceRequestForm() {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<ServiceRequestInput>({
    ...
    defaultValues: { name: "", email: "", phone: "", serviceType: "windows-linux", description: "", preferredDate: "", consent: false, company: "" },
  });
  ...
  if (serverState?.type === "success") return <div className="success-state" role="status" data-testid="request-success"><CheckCircle2 size={34} /><h2>Poptávka je bezpečně uložená</h2><p>{serverState.message}</p><strong>Číslo poptávky: {serverState.reference}</strong><button className="button button-secondary" onClick={() => setServerState(null)}>Odeslat další poptávku</button></div>;
  return (
    <form className="form-card request-form" onSubmit={handleSubmit(submit)} noValidate data-testid="service-request-form">
      ...
      <div className="form-grid">
        ...
        <label className="form-span"><span>Popis problému *</span><textarea {...register("description")} rows={5} placeholder="Co přesně nefunguje, jaké zařízení používáte a co už jste zkusili?" aria-invalid={!!errors.description} />{errors.description && <small className="field-error">{errors.description.message}</small>}</label>
