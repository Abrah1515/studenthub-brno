...
import { CalendarDays, Flag, MessageCircle, PlusCircle, ShoppingBag, Users } from "lucide-react";
...
export const tutorialVersion = "studenthub-marketplace-v4";
...
export function FeatureTutorial() {
  ...
  return <div className="onboarding-backdrop tutorial-overlay" data-modal-layer><div ref={dialogRef} tabIndex={-1} className="tutorial-card" role="dialog" aria-modal="true" aria-labelledby="feature-tutorial-title" aria-describedby="feature-tutorial-description" data-modal-layer><div className="tutorial-title-icon"><BrandSymbol size={46} /></div><span className="eyebrow">{full ? "Vítejte ve StudentHubu" : "Průvodce StudentHubem"}</span><h2 id="feature-tutorial-title">Každý typ komunitního obsahu má své místo</h2><p id="feature-tutorial-description">StudentHub spojuje ověřené akademické termíny, užitečná místa, bezpečnou studentskou komunitu a burzu. Školu, fakultu a ročník můžete kdykoli změnit v části Moje škola a profil.</p><div className="tutorial-steps"><article><span><CalendarDays size={20} /></span><div><strong>Co se děje</strong><p>Veřejné studentské akce s konkrétním datem najdete v kalendáři.</p></div></article><article><span><Users size={20} /></span><div><strong>Hledám parťáka</strong><p>Slouží k domluvení společné aktivity na konkrétní čas.</p></div></article><article className="tutorial-highlight"><span><ShoppingBag size={20} /></span><div><strong>Studentská burza</strong><p>Učebnice, vlastní poznámky a vybavení. Nabídku zveřejní přihlášený uživatel s dokončeným profilem.</p></div></article><article><span><MessageCircle size={20} /></span><div><strong>Studentská komunita</strong><p>Feed pro otázky, rady, zkušenosti a tipy napříč brněnskými školami.</p></div></article><article><span><PlusCircle size={20} /></span><div><strong>Bezpečné publikování</strong><p>Kontaktní e-mail burzy není veřejný a zájemce píše přes serverový relay.</p></div></article><article><span><Flag size={20} /></span><div><strong>Nahlášení obsahu</strong><p>Nelegální kopie, podvodné nabídky a porušení akademické poctivosti lze nahlásit.</p></div></article></div><div className="tutorial-actions"><button className="button button-primary" data-autofocus onClick={close}>Rozumím</button></div></div></div>;
}
