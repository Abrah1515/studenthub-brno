...
export function EventExplorer({ events, initialUniversityId = "", initialFacultyId = "", cityId }: { events: AcademicEvent[]; initialUniversityId?: string; initialFacultyId?: string; cityId: string }) {
  ...
  return <>
    ...
    <section className="result-stack" aria-live="polite"><div className="result-count"><strong>{filtered.length}</strong> ověřených událostí</div>
      {filtered.length === 0 ? <div className="empty-state"><CalendarPlus size={26} /><h2>Žádný termín neodpovídá filtrům</h2><p>Aktivní filtry: {[query, category !== "Všechny" ? category : "", universityId, facultyId].filter(Boolean).join(" · ") || "žádné"}.</p><button className="button button-secondary" onClick={resetFilters}>Resetovat filtry</button></div> : filtered.map((event) => { const freshness = eventFreshness(event.lastVerifiedAt); return <article className={`result-card event-card ${event.changeState === "changed" ? "event-changed" : ""}`} key={event.id} id={event.id}>
