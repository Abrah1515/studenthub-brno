...
export function EventExplorer({ events, initialUniversityId = "", initialFacultyId = "", cityId }: { events: AcademicEvent[]; initialUniversityId?: string; initialFacultyId?: string; cityId: string }) {
  ...
  const filtered = useMemo(() => events.filter((event) => includesFolded(`${event.title} ${event.description}`, query) && (category === "Všechny" || event.category === category) && matchesSelection(event, universityId, facultyId)), [events, query, category, universityId, facultyId]);
