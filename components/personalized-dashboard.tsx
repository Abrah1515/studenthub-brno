...
export function DashboardHeading({ city, selection }: { city: City; selection?: StudySelection }) {
  ...
  return <header className="page-heading"><div><span className="eyebrow">{today}</span><h1>{university ? `Přehled pro ${faculty?.shortName || university.shortName}` : `StudentHub ${city.name}`}</h1><p>{university ? `${university.name}${faculty ? ` · ${faculty.name}` : ""}. K tomu obsah užitečný pro celé město ${city.name}.` : `Ověřené termíny, místa a praktické služby pro studenty ve městě ${city.name}.`}</p></div><div className="heading-actions"><Link href={`/${city.slug}/nastaveni`} className="button button-secondary"><Settings2 size={18} />Moje škola</Link><Link href={`/${city.slug}/burza`} className="button button-primary"><ShoppingBag size={18} />Studentská burza</Link></div></header>;
}
...
export function PersonalizedDashboard({ events, offers, jobs, city, selection }: { events: AcademicEvent[]; offers: Offer[]; jobs: Job[]; city: City; selection?: StudySelection }) {
  ...
  if ((!selection && !preference.completed) || !university) return <section className="personalized-empty"><GraduationCap size={21} /><div><h2>Přizpůsobte si StudentHub</h2><p>Vyberte školu a fakultu bez registrace. Doporučení pak budou konkrétnější.</p></div><Link href={`/${city.slug}/nastaveni`} className="button button-secondary">Vybrat školu <ArrowRight size={16} /></Link></section>;
  return <section className="personalized-panel"><div className="personalized-head"><div><span className="eyebrow">Pro vás · {faculty?.shortName || university.shortName}</span><h2>Výběr podle vaší školy</h2></div><Link href={`/${city.slug}/nastaveni`}>Změnit výběr</Link></div><div className="personalized-grid"><div><span className="section-icon"><CalendarDays size={17} /></span><strong>{personalized.events[0]?.title || `Společné termíny · ${city.name}`}</strong><small>{personalized.events[0] ? formatShortDate(personalized.events[0].start) : "Průběžně doplňujeme"}</small></div>{featureFlags.offersEnabled && personalized.offers.length > 0 && <div><span className="section-icon"><Sparkles size={17} /></span><strong>{personalized.offers[0].title}</strong><small>{personalized.offers[0].partner}</small></div>}<div><span className="section-icon"><BriefcaseBusiness size={17} /></span><strong>{personalized.jobs[0]?.title || `Brigády · ${city.name}`}</strong><small>{personalized.jobs[0]?.company || "Napříč obory"}</small></div><Link href={university.slug === "muni" || university.slug === "vut" || university.slug === "mendelu" ? `/${city.slug}/skoly/${university.slug}` : `/${city.slug}/mista`}><MapPinned size={18} /><span>Školní rozcestník</span><ArrowRight size={16} /></Link></div></section>;
}
