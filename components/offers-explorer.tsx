...
export function OffersExplorer({ items }: { items: Offer[] }) {
  ...
  return <>
    ...
    <MobileFilterDialog open={filtersOpen} activeCount={activeFilterCount} onClose={() => setFiltersOpen(false)} onReset={resetFilters} controlsId="offers-mobile-filters" bodyClassName="offers-filters" applyLabel={`Zobrazit ${filtered.length} nabídek`}>{filterControls}</MobileFilterDialog>
