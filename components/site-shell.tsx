...
function navigationFor(citySlug: string, cityName: string) {
  ...
  return [
    ...
    { href: `${cityBase}/hlidac`, label: "Hlídač", short: "Hlídač", icon: Bell },
    { href: `${cityBase}/chat`, label: "Chat", short: "Chat", icon: MessageCircle },
    { href: `${cityBase}/mista`, label: `Místa – ${cityName}`, short: "Místa", icon: MapPinned },
    { href: `${cityBase}/komunita`, label: "Studentská komunita", short: "Komunita", icon: MessageCircle },
    { href: `${cityBase}/partak`, label: "Hledám parťáka", short: "Parťák", icon: Users },
    ...
    { href: `${cityBase}/burza`, label: "Studentská burza", short: "Burza", icon: ShoppingBag },
    ...(citySlug === "brno" ? [{ href: `${cityBase}/bydleni`, label: "Bydlení", short: "Bydlení", icon: Building2 }] : []),
  { href: `${cityBase}/nastaveni`, label: "Moje škola a profil", short: "Profil", icon: Settings },
  ];
}
...
function PreferenceAwareNavLink({ item, pathname, cityRoot, close, compact = false }: { item: NavigationItem; pathname: string; cityRoot: string; close?: () => void; compact?: boolean }) {
  ...
  return <Link href={href} onClick={navigate} className={compact ? classNames(active && "active") : classNames("nav-link", active && "nav-link-active")} aria-current={active ? "page" : undefined}><Icon size={compact ? 20 : 19} aria-hidden="true" /><span>{compact ? item.short : item.label}</span>{item.href.endsWith("/hlidac") && <WatcherBadge />}{item.href.endsWith("/chat") && <ChatBadge compact={compact} />}</Link>;
}
...
function MobileMenu({ open, close, navigation, pathname, cityRoot, returnFocus }: { open: boolean; close: () => void; navigation: ReturnType<typeof navigationFor>; pathname: string; cityRoot: string; returnFocus: () => HTMLElement | null }) {
  ...
  return createPortal(<div className="mobile-menu-layer" data-modal-layer><button className="mobile-menu-backdrop" data-modal-layer aria-label="Zavřít nabídku" onClick={close} /><aside ref={ref} tabIndex={-1} className="mobile-menu-panel" aria-label="Mobilní nabídka" role="dialog" aria-modal="true" data-modal-layer><div className="sidebar-head"><Brand href={cityRoot} /><button className="icon-button" data-autofocus aria-label="Zavřít nabídku" onClick={close}><X size={20} /></button></div><nav className="desktop-nav tablet-full-nav" aria-label="Hlavní navigace">{navigation.map((item) => <PreferenceAwareNavLink key={item.href} item={item} pathname={pathname} cityRoot={cityRoot} close={close} />)}</nav><nav className="phone-extra-nav" aria-label="Doplňkové funkce"><Link className="nav-link" href={`${cityRoot}/chat`} onClick={close}><MessageCircle size={19} />Chat<ChatBadge /></Link><Link className="nav-link" href={`${cityRoot}/bydleni`} onClick={close}><Building2 size={19} />Bydlení</Link><Link className="nav-link" href={`${cityRoot}/hlidac`} onClick={close}><Bell size={19} />Hlídač<WatcherBadge /></Link><Link className="nav-link" href={`${cityRoot}/nastaveni`} onClick={close}><Settings size={19} />Moje škola a profil</Link><Link className="nav-link" href="/" onClick={close}><MapPinned size={19} />Změnit město</Link><div className="nav-link install-nav-item"><PwaInstallButton onBeforeOpen={close} returnFocus={returnFocus} /></div><button className="nav-link" type="button" onClick={() => { close(); window.dispatchEvent(new Event(openTutorialEvent)); }}><BookOpen size={19} />Návod</button><div className="mobile-theme-block"><strong>Nastavení vzhledu</strong><ThemeSettings /></div><Link className="nav-link" href="/o-projektu" onClick={close}>O projektu</Link><Link className="nav-link" href="/kontakt" onClick={close}>Kontakt</Link><Link className="nav-link" href="/admin" onClick={close}>Administrace</Link></nav><div className="sidebar-note"><Info size={18} /><p>Nezávislý projekt. Není oficiálně spojený s žádnou univerzitou.</p></div><nav className="sidebar-legal tablet-menu-extras" aria-label="Doplňkové odkazy"><Link href="/" onClick={close}><MapPinned size={15} />Změnit město</Link><Link href="/o-projektu" onClick={close}>O projektu</Link><PwaInstallButton onBeforeOpen={close} returnFocus={returnFocus} /><button type="button" onClick={() => { close(); window.dispatchEvent(new Event(openTutorialEvent)); }}><BookOpen size={15} />Návod</button><Link href="/kontakt" onClick={close}>Kontakt</Link><Link href="/admin" onClick={close}>Administrace</Link></nav></aside></div>, document.body);
}
