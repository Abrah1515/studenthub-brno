...
function ThemeSettings() {
  ...
  useEffect(() => { const stored = localStorage.getItem("studenthub-theme") as Theme | null; const initial = stored && ["system", "light", "dark"].includes(stored) ? stored : "system"; setTheme(initial); applyTheme(initial); const media = matchMedia("(prefers-color-scheme: dark)"); const update = () => (localStorage.getItem("studenthub-theme") || "system") === "system" && applyTheme("system"); media.addEventListener("change", update); return () => media.removeEventListener("change", update); }, []);
  ...
  return <div className="theme-settings" role="radiogroup" aria-label="Nastavení vzhledu">{options.map(({ value, label, icon: Icon }) => <button key={value} type="button" role="radio" data-theme-option={value} className={theme === value ? "active" : ""} aria-checked={theme === value} onClick={() => { setTheme(value); localStorage.setItem("studenthub-theme", value); applyTheme(value); }}><Icon size={17} /><span>{label}</span></button>)}</div>;
}
...
export function SiteShell({ children, cities, catalog }: { children: React.ReactNode; cities: City[]; catalog: AcademicCatalog }) {
  ...
  return <AcademicCatalogProvider catalog={catalog}><div className="app-shell">
    ...
    <div className="main-column"><header className="topbar"><button ref={menuTriggerRef} className="icon-button mobile-only" aria-label="Otevřít nabídku" onClick={() => setMenuOpen(true)}><Menu size={20} /></button><div className="mobile-brand"><Brand href={cityRoot} /></div><div className="topbar-spacer" /><Link href="/pomoc" className="button button-primary topbar-help" aria-label="Technická pomoc"><Wrench size={18} /><span>Technická pomoc</span></Link></header><main id="hlavni-obsah" className="content">{children}</main><footer className="footer"><p>{brand.editionName} · nezávislý studentský projekt</p><div><Link href="/soukromi">Soukromí</Link><Link href="/cookies">Cookies</Link><Link href="/podminky">Podmínky</Link><button type="button" onClick={() => window.dispatchEvent(new Event("open-cookie-settings"))}>Nastavení cookies</button></div></footer></div>
