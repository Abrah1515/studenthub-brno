...
function navigationFor(citySlug: string, cityName: string) {
  ...
  return [
    ...
    { href: "/chat", label: "Chat", short: "Chat", icon: MessageCircle },
  ...
  { href: "/nastaveni", label: "Moje škola a profil", short: "Profil", icon: Settings },
  ];
}
...
function isActive(pathname: string, href: string, cityRoot: string) { return pathname === href || (["/komunita","/chat"].includes(href) && pathname.startsWith(`${href}/`)) || (href === cityRoot && pathname === "/"); }
...
export function SiteShell({ children, cities, catalog }: { children: React.ReactNode; cities: City[]; catalog: AcademicCatalog }) {
  ...
  const chatConversationOpen = /^\/chat\/[^/]+/.test(pathname);
