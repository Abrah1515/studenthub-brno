...
import { CalendarDays, MessageCircle, ShoppingBag, Trash2, Users } from "lucide-react";
...
export function AccountContentSummary({ content, onRefresh }: { content: Content; onRefresh: () => void }) {
  ...
  return <div className="account-content-summary">
    ...
    <section><h3><CalendarDays size={17} /> Komunitní akce ({content.events?.length || 0})</h3>{content.events?.length ? content.events.map((item) => <div key={String(item.id)}><Link href={`/brno/kalendar?view=community#${item.id}`}>{String(item.title)}</Link><button className="button button-quiet" onClick={() => removeEvent(String(item.id))}><Trash2 size={14} /> Zrušit</button></div>) : <p>Žádná vlastní akce.</p>}</section>
  </div>;
}
