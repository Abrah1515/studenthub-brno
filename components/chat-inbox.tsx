...
export function ChatInbox() {
  ...
  return <div className="chat-page"><div className="page-heading"><div><p className="eyebrow">Soukromé zprávy</p><h1>Chat</h1><p>Konverzace jsou viditelné jen jejich účastníkům. StudentHub nepoužívá end-to-end šifrování.</p></div><button className="button button-secondary" onClick={refresh}><RefreshCw size={16} />Obnovit</button></div>{target && <ChatComposerCard target={target} onClose={() => router.replace("/brno/chat")} />}
    ...
    {loading ? <div className="chat-list-state">Načítám konverzace…</div> : error ? <div className="chat-list-state"><X /><p>{error}</p><Link className="button button-primary" href={`/ucet/prihlaseni?next=${encodeURIComponent("/brno/chat")}`}>Přihlásit se</Link></div> : items.length === 0 ? <div className="chat-list-state"><MessageCircle size={34} /><h2>{tab === "requests" ? "Žádné nové žádosti" : "Zatím tu nic není"}</h2><p>Konverzaci zahájíte tlačítkem Napsat u veřejného profilu, příspěvku Hledám parťáka nebo burzovního inzerátu.</p></div> : <div className="chat-list">{items.map((item) => <Link key={item.id} href={`/brno/chat/${item.id}`} className={item.unreadCount ? "chat-list-item unread" : "chat-list-item"}><span className="chat-avatar">{item.other.avatarUrl ? <img src={item.other.avatarUrl} alt="" /> : <UserRound size={20} />}</span><span className="chat-list-copy"><strong>{item.other.displayName}</strong><small>{item.lastMessage?.body || item.context.title}</small><em>{item.context.active ? item.context.title : "Původní nabídka již není dostupná"}</em></span><span className="chat-list-meta"><time>{new Intl.DateTimeFormat("cs-CZ", { day: "numeric", month: "numeric" }).format(new Date(item.updatedAt))}</time>{item.unreadCount > 0 && <b aria-label={`${item.unreadCount} nepřečtených`}>{item.unreadCount}</b>}</span></Link>)}</div>}
  </div>;
}
