...
type OwnedJobProposal={id:string;status:string;content:Partial<JobSubmissionInput>;moderationNote?:string|null};
function JobProposal({ onClose, item }: { onClose: () => void; item?:OwnedJobProposal|null }) {
  const [success, setSuccess] = useState(false); const [serverError, setServerError] = useState("");
  const dialogRef = useModalDialog(true, onClose);
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<z.input<typeof jobSubmissionSchema>, unknown, JobSubmissionInput>({ resolver: zodResolver(jobSubmissionSchema), defaultValues: { reward: item?.content.reward||160,consent:false,company:"",companyName:item?.content.companyName||"",title:item?.content.title||"",contactEmail:item?.content.contactEmail||"",location:item?.content.location||"",workload:item?.content.workload||"",description:item?.content.description||"" } });
  async function submit(data: JobSubmissionInput) {
    setServerError(""); const response = await fetch(item?`/api/jobs/${item.id}`:"/api/jobs", { method: item?"PATCH":"POST", headers: { "content-type": "application/json" }, body: JSON.stringify(data) }); const result = await response.json();
    if (!response.ok) return setServerError(result.message || "Návrh se nepodařilo uložit."); setSuccess(true);
  }
  return <div ref={dialogRef} tabIndex={-1} className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="job-dialog-title" data-modal-layer><div className="modal-card"><div className="modal-head"><div><span className="eyebrow">Pro firmy a týmy</span><h2 id="job-dialog-title">{item?"Upravit návrh brigády":"Navrhnout brigádu"}</h2></div><button className="icon-button" data-autofocus onClick={onClose} aria-label="Zavřít formulář"><X size={19} /></button></div>{success ? <div className="success-state"><CheckCircle2 size={32} /><h3>Návrh je uložený</h3><p>Inzerát se zveřejní až po kontrole administrátorem.</p><button className="button button-primary" onClick={onClose}>Hotovo</button></div> : <form className="form-grid" onSubmit={handleSubmit(submit)} noValidate><label><span>Firma *</span><input {...register("companyName")} />{errors.companyName && <small className="field-error">Alespoň 2 znaky.</small>}</label><label><span>Název pozice *</span><input {...register("title")} />{errors.title && <small className="field-error">Alespoň 4 znaky.</small>}</label><label><span>Kontaktní e-mail *</span><input type="email" {...register("contactEmail")} />{errors.contactEmail && <small className="field-error">Zadejte platný e-mail.</small>}</label><label><span>Lokalita *</span><input {...register("location")} />{errors.location && <small className="field-error">Doplňte lokalitu.</small>}</label><label><span>Odměna v Kč/h *</span><input type="number" {...register("reward")} />{errors.reward && <small className="field-error">Minimálně 100 Kč.</small>}</label><label><span>Časová náročnost *</span><input {...register("workload")} placeholder="Např. 10 h týdně" />{errors.workload && <small className="field-error">Doplňte rozsah.</small>}</label><label className="form-span"><span>Popis práce *</span><textarea rows={4} {...register("description")} />{errors.description && <small className="field-error">Alespoň 30 znaků.</small>}</label><label className="honeypot" aria-hidden="true"><span>Company</span><input tabIndex={-1} autoComplete="off" aria-hidden="true" {...register("company")} /></label><label className="checkbox-field form-span"><input type="checkbox" {...register("consent")} /><span>Souhlasím se zpracováním údajů pro vyřízení návrhu. *</span></label>{errors.consent && <small className="field-error form-span">Potvrďte souhlas.</small>}{serverError && <p className="field-error form-span">{serverError}</p>}<LegalNotice privacyOnly /><button className="button button-primary form-span" disabled={isSubmitting}><Send size={17} />{isSubmitting ? "Ukládám…" : item?"Uložit a odeslat ke kontrole":"Odeslat ke schválení"}</button></form>}</div></div>;
}
...
export function JobExplorer({ items }: { items: Job[] }) {
  ...
  useEffect(()=>{if(!myOpen)return;fetch("/api/jobs?scope=mine",{cache:"no-store"}).then(async(response)=>{const payload=await response.json().catch(()=>({}));if(!response.ok)throw new Error(payload.message||"Návrhy se nepodařilo načíst.");setMyItems(payload.items||[])}).catch((error)=>setMyMessage(error instanceof Error?error.message:"Návrhy se nepodařilo načíst."))},[myOpen,showProposal,editing]);
  ...
  return <>
    ...
    {showProposal && <JobProposal onClose={() => setShowProposal(false)} />}{editing&&<JobProposal item={editing} onClose={()=>setEditing(null)}/>}
  </>;
}
