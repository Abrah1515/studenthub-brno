...
export function ChatComposerCard({ target, onClose, dock = false }: { target: ChatComposerTarget; onClose: () => void; dock?: boolean }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!response?.ok) { if (response?.status === 401) router.push(`/ucet/prihlaseni?next=${encodeURIComponent(location.pathname)}`); else if (response?.status === 428) router.push("/brno/nastaveni"); else setError(body?.message || "Žádost se nepodařilo odeslat."); return; }
    const id = body.conversation?.id; if (!id) return;
    if (dock) window.dispatchEvent(new CustomEvent("studenthub-open-chat", { detail: { id } })); else router.replace(`/brno/chat/${id}`);
  }
